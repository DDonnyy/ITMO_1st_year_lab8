...
public class ExecuteScript implements Command {
    ...
    public void execute(String par1) throws IOException {
        try {
            ...
            if (numberOfExecution==0){ fileNames.clear();inExecution = false;ServerSender.send("",0);}

        } catch (AccessDeniedException ex) {
            if (ExecuteScript.inExecution) ServerSender.send("Нет доступа к файлу.",2);
            else ServerSender.send("Нет доступа к файлу.",0);
        }
        catch (NullPointerException ex){
            if (ExecuteScript.inExecution) ServerSender.send("Имя файла не указано или файл пустой.",2);
            else ServerSender.send("Имя файла не указано или файл пустой.",0);

        }
        catch (FileNotFoundException ex){
            if (ExecuteScript.inExecution) ServerSender.send("Файл не найден,попробуйте ещё раз.",2);
