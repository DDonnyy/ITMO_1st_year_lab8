...
public class FilterByPrice implements Command {
    ...
    public void execute(String par1) throws IOException {
        try { if(par1==null&& ExecuteScript.inExecution){
            ServerSender.send("Выполнение скрипта: Цена для усреднения не была указана,выполнение команды невозможно.",2);
        }
          else  if (par1 == null){
                String key;
            ...
            if (key.equals("") || key == null || key.equals("\n")) { ServerSender.send("Цена не может быть null",2);this.execute(par1);}
               ...
               if (!ExecuteScript.inExecution) ServerSender.send("",0);
            }
        } catch (NumberFormatException|NullPointerException e){
            if (ExecuteScript.inExecution) ServerSender.send("Цена указана некорректно,попробуйте ещё раз",2);
                else ServerSender.send("Цена указана некорректно,попробуйте ещё раз",0);
        }
    }
